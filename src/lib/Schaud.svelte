...
// our loop
    ...
    $effect(() => {
        if (ready) scheduleNextSound(cuelets[0])
    })
    let upto = $state(0)
    let theone = {}
    function scheduleNextSound(cuelet:adublet,the) {
        if (the && the != theone) return
        the = theone = {}
        upto = cuelet.in
        console.log(`NextSound: ${cuelet.in}  ${cuelet.buffer.length}`)
        ...
        source.onended = () => {
            let cuenext = next_cuelet(cuelet)
            if (cuenext.in < cuelet.out) {
                console.log("Wrap")
            }
            scheduleNextSound(cuenext,the);
        };
    }
    function oscil() {
